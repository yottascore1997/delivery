          ...
          {qty > 0 ? (
            <div className="flex items-center justify-between rounded-lg border border-violet-200 bg-violet-50 px-2 py-1">
              <button
                ...
                className="h-9 w-9 rounded-md text-xl font-black text-violet-700 hover:bg-violet-100"
              >
                -
              </button>
              <span className="min-w-[2rem] text-center text-base font-black text-violet-800">{qty}</span>
              <button
                type="button"
                disabled={storeClosed}
                onClick={() => updateShopLineQty(item.id, qty + 1)}
                className="h-9 w-9 rounded-md text-xl font-black text-violet-700 hover:bg-violet-100 disabled:cursor-not-allowed disabled:opacity-35"
              >
                +
              </button>
            </div>
            ...
            <button
              ...
              className="w-full rounded-lg border border-emerald-700 bg-emerald-600 py-2.5 text-sm font-black text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40"
            >
        ...
        {suggested.length > 0 ? (
          <div className="mt-5 border-t border-slate-200 pt-4">
            ...
            <ul className="mt-3 space-y-2.5">
              {suggested.map((s) => (
                <li key={s.id} className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                  <div className="flex gap-2.5">
                    <Link href={`/shop/product/${s.id}`} className="h-12 w-12 overflow-hidden rounded-lg border border-slate-200 bg-white">
                      <ProductThumb name={s.name} imageUrl={s.imageUrl} className="h-full w-full object-cover" />
                    </Link>
                    <div className="min-w-0 flex-1">
                      <Link
                        href={`/shop/product/${s.id}`}
                        className="line-clamp-1 text-xs font-bold text-slate-900 hover:text-violet-700"
                      >
                        {s.name}
                      </Link>
                      <p className="mt-0.5 text-xs font-black text-slate-900">
                        ₹{Math.round(s.price)}
                        {s.unitLabel?.trim() ? (
                          <span className="ml-1.5 font-semibold text-slate-500">· {s.unitLabel.trim()}</span>
                        ) : null}
                      </p>
                    </div>
                  </div>
                    ...
                    {qtyFor(s.id) > 0 ? (
                      <div className="flex items-center justify-between rounded-lg border border-violet-200 bg-violet-50 px-2 py-1">
                        <button
                          ...
                          className="h-7 w-7 rounded text-lg font-black text-violet-700 hover:bg-violet-100"
                        >
                          -
                        </button>
                        <span className="min-w-[1.5rem] text-center text-xs font-black text-violet-800">
                          {qtyFor(s.id)}
                        </span>
                        <button
                          type="button"
                          disabled={storeClosed}
                          onClick={() => updateShopLineQty(s.id, qtyFor(s.id) + 1)}
                          className="h-7 w-7 rounded text-lg font-black text-violet-700 hover:bg-violet-100 disabled:cursor-not-allowed disabled:opacity-35"
                        >
                          +
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full rounded-lg border border-emerald-700 bg-emerald-600 py-1.5 text-xs font-black text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {storeClosed ? "Store closed" : "Add To Cart"}
                      </button>
                    )}
